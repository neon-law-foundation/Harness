...
@main
struct HarnessAPIServer {
    static func main() async throws {
        let environment = ProcessInfo.processInfo.environment["env"] ?? "development"

        // Log the detected environment
        let logger = Logger(label: "com.harness.server")
        logger.info("Starting Harness API Server", metadata: ["environment": "\(environment)"])
...
private func configureDatabase(_ app: Application) async throws {
    ...
    // Load seed data
    let seedCount = try await HarnessDALConfiguration.runSeeds(
        on: app.db,
        logger: app.logger
    )
...
private func configureRoutes(_ app: Application) async throws {
    ...
    let handler = HarnessAPIHandler(logger: app.logger)
...
struct HarnessAPIHandler: APIProtocol {
    let logger: Logger
