...
to either a `PERSON` **OR** an `ENTITY`, but never both.
...
- `referenced_by` (enum): Indicates which entity type (letters |
  shareIssuances | answers)
- `referenced_by_id` (int32): The ID of the referenced entity

This allows flexible attachment of blobs to different entity types without
...
on the notation's `respondent_type`:
...
The `NOTATION` entity includes a `version` field storing the git commit SHA
from the source repository. This enables:
...
This ensures only one active assignment exists for each notation-respondent
combination.
