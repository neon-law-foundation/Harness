...
///
/// Wraps the shared `HarnessAPIImplementation` and integrates with
/// AWS Lambda Runtime via OpenAPILambda transport.
private struct LambdaAdapter: APIProtocol, OpenAPILambdaHttpApi {
    let implementation: HarnessAPIImplementation

    init() async throws {
        self.implementation = HarnessAPIImplementation()
    }
...
private func runVapor() async throws {
    ...
    fatalError(
        "Vapor runtime not available in HarnessAPI target. Use HarnessAPIServer instead."
    )
}
